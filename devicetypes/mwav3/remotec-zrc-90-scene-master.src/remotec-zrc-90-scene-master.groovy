 ...
 *
 *  Contains code from https://github.com/erocm123/SmartThingsPublic/blob/master/devicetypes/erocm123/remotec-zrc-90-scene-master.src/remotec-zrc-90-scene-master.groovy
 *
 *  Copyright 2020 Eric Maycock, Tim Grimley
 *
 *  This is an update from Tim Grimley of the original DTH made by Eric Maycock, who advised he no longer has the device but stated he was open to others
 *  assisting with an update.  Thank you Eric for all the work done to initially put this together!
 *
 ...
 *
 *  0.11 (01/12/2021) - Initial Release Old handler Updated to add compatibility to new Smart lighting app and webcore
 *
 *  Note - This release will not work with the native automations part of the new smartthings app, which currently only supports one button.
 *  Due to this device having 8 buttons with 3 different actions, for full functionality of the device use of webcore or the smartlighting app
 *  will be required for this device handler to fully function with all button presses and actions
 *
 *  If button values do not change on initial install, go to preferences and toggle "Force Settings Update/Refresh"
 * 
 *  When using webcore, "double" is not a default option.  You will have to enter as an expression "double" for Webcore to recognize double taps.
 *
 *  Button Mappings  
 *
...
metadata {
	definition (name: "Remotec ZRC-90 Scene Master", namespace: "mwav3", author: "Tim Grimley", ocfDeviceType: "x.com.st.d.remotecontroller") {
		capability "Actuator"
		capability "Button"
...
def zwaveEvent(physicalgraph.zwave.commands.centralscenev1.CentralSceneNotification cmd) {
        ...
        buttonnum = (cmd.sceneNumber) as Integer
...
def configure() {
    ...
    sendEvent(name: "supportedButtonValues", value:JsonOutput.toJson(["pushed","held","double"]), displayed:false)
